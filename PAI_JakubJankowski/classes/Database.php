...
class Database {
	...
	public function selectOrder($sql) {
		...
		if ($result = $this->mysqli->query($sql)) {
			...
			$content .= "<div class='table-responsive'><table class='table table-bordered'><tbody><tr class='table-primary'>";
	...
	public function selectOrderAdmin($sql) {
		$content = "";
		$fields = ['orderId', 'userId', 'kursy', 'fullName', 'email', 'phone', 'address', 'delivery', 'total', 'orderDate'];
		$pola = ['Nr zamówienia', 'Id użytkownika', 'Kursy', 'Imię i nazwisko', 'Email', 'Telefon', 'Adres', 'Dostawa', 'Suma', 'Data'];
		if ($result = $this->mysqli->query($sql)) {
			...
			while ($row = $result->fetch_object()) {
				...
				$content .= "<td><a href='history.php?action=$row->orderId'><i class='bi bi-x fs-4'></i></a></td>";
				$content .= "</tr>";
			}
			$content .= "</tbody></table></div>";
			$result->close();
	...
	public function selectUserTable($sql) {
		...
		if ($result = $this->mysqli->query($sql)) {
			...
			while ($row = $result->fetch_object()) {
				...
				if($row->status != 2) {
					$content .= "<td><a href='userData.php?action=$row->id'><i class='bi bi-x fs-4'></i></a></td>";
					$content .= "</tr>";
				} else {
					$content .= "<td></td>";
					$content .= "</tr>";
